/**Ascenso en la montaña para el problema de RBM*/
#include <iostream>
#include <vector>
#include <cmath>
#include <random>
...
void genera_sol(int **coor_rob, int cap[], int n, vector <vector <int>> &new_pos, int rob[], vector <int> &sol, int &dis){
  ...
  for(int k = 0; k < n; k++){
    if(cap[rob[k]] > abs(coor_rob[rob[k]][0] - bx) + abs(coor_rob[rob[k]][1] - by)){ //Si el robot en la posicion k puede llegar a la posicion actual de la bomba
      ...
      new_dis = mov + abs(bx - new_pos[0][0]) + abs(by - new_pos[1][0]); //Unidades de alejamiento (F.O.) que puede lograr el robot en la pos. k
      if(new_dis > dis){
        p += 1;
...
int main(){
  ...
  int **coor_rob, cap[n]; //Coordenadas de los robots y capacidad de energia
  coor_rob = new int *[n];
  for(int i = 0; i < n; i++){
  ...
  for(int iter = 0; iter < corridas; iter++){
    ...
    /**Asignacion de variables y vectores a comparar y reinicio de los primeros*/
    int dis_fin = dis;
    ...
    cout << "\nLos movimientos son:\n";
    for(int i = 0; i < pos_fin[0].size(); i++){
